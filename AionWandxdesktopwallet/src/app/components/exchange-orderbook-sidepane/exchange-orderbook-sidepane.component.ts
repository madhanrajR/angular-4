...
exchange:any;
 ...
 //   console.log(order)
  ...
  if(sessionStorage.getItem('exchange')=='aion')
  {
    ...
    var sig = [
    this.selectedSellOrder.SellerAIONAddress,
    '0x'+this.selectedSellOrder.SellerSign.substr(0,64),
    '0x'+this.selectedSellOrder.SellerSign.substr(64,128)
    ]
    var hash=this.selectedSellOrder.SellerHash;
    var blocknumber=this.selectedSellOrder.ExpiringBlock;
    console.log(sig,hash)
console.log("Getting gas estimate");
const contractFunction = instanceOrderTraderContract.methods.fillOrder(tokensAndAddresses,volumes,blocknumber,1,hash,sig)
...
const txParams = {
 ...
 self._web3.eth.sendSignedTransaction(res.rawTransaction).on('transactionHash',hash => {
  ...
  if(sessionStorage.getItem('exchange')=='aion')
  {
   ...
   // var ecSignature = web3Functions.extractECSignature(this.selectedBuyOrder.BuyerSign, this.selectedBuyOrder.BuyerHash, this.selectedBuyOrder.BuyerAccountId);
    ...
    this.isSellModalVisible = false;
    var sig=[
    this.selectedBuyOrder.BuyerAIONAddress,
    '0x'+this.selectedBuyOrder.BuyerSign.substr(0,64),
    '0x'+this.selectedBuyOrder.BuyerSign.substr(64,128)
    ]
    console.log(sig)
    const privateKey = this.web3Service.priv;
    const contractFunction = instanceOrderTraderContract.methods.fillOrder(tokensAndAddresses, volumes, this.selectedBuyOrder.ExpiringBlock,0,this.selectedBuyOrder.BuyerHash,sig)
  const functionAbi = contractFunction.encodeABI();
  this.showLoader = true;
      this.displayGif = 'block';
  const txParams = {
 ...
 // console.log('error')
  ...
  if(sessionStorage.getItem('exchange')=='aion')
  {
    if (this.selectedSellOrder.FeeToken.address === Constants.WandxTokenAddressWAN) {
      return this.selectedSellOrder.TargetVolume * this.selectedSellOrder.SellingVolume * 3792;
    }
    else {
      return this.selectedSellOrder.TargetVolume * this.selectedSellOrder.SellingVolume;
  ...
  if(sessionStorage.getItem('exchange')=='aion')
  {
    if (this.selectedBuyOrder.FeeToken.address === Constants.WandxTokenAddressWAN) {
      return this.selectedBuyOrder.TargetVolume * this.selectedBuyOrder.BuyingVolume * 3792;
    }
    else {
      return this.selectedBuyOrder.TargetVolume * this.selectedBuyOrder.BuyingVolume;
