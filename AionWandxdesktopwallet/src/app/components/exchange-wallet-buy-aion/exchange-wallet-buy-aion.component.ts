...
createBuyOrder() {
  ...
  this._web3.eth.getBlockNumber((err, data) => {
    if (data) {
      ...
      var result=this._web3.eth.accounts.sign([Constants.OrderBookContractAddressWAN,sellTokenAddress, buyTokenAddress, buyVolume, buyPrice,creationBlock + Constants.BlockExpirationWindow, userAccount, 0,sanitizedOrderId,buyerFeeTokenAddress],privatekey)
