 import { Component, OnInit,OnDestroy, NgZone, Self } from '@angular/core';
...
export class AionwalletComponent implements  OnInit, OnDestroy {
  ...
  constructor(private navService: NavigationService,
             ...
             // private ww  : JSONAionWallet,
              ...
              //  this.exchangeWalletSelectComponent=new ExchangeWalletSelectComponent()
                var self=this;
...
//  });
  ...
  deposit() {
   ...
   var userAccount = sessionStorage.getItem('walletAddress');
    ...
    self.wizard1=false;
   self.ccadd=self.savedWalletsService.getCurrentWallet();
   console.log("selected Account Address",self.ccadd)
   var privatekey=self.aionWalletHelper.decrypted(self.ccadd.wallet.wallet.data,p1,true)
console.log("deposit private key",privatekey)
    ...
    self.notificationsService.showNotification(new MessageModel(MessageType.Info, 'Depositing ' + this.amount + ' as WXETH to enable trading in ERC20 Token Baskets'), MessageContentType.Text);
    ...
    // this.instance.methods.deposit(userAccount, {from: userAccount, value: convertedAmount}, (err, result) => {
 ...
 var p1=p.toString();
  var self=this;
 self.ccadd=this.savedWalletsService.getCurrentWallet();
 let web3Instance = self.web3.getWeb3();
 var userAccount = sessionStorage.getItem('walletAddress');
 console.log("selected Account Address",self.ccadd)
 var privatekey=self.aionWalletHelper.decrypted(self.ccadd.wallet.wallet.data,p1,true)
...
if(res==0){
  self.notificationsService.showNotification(new MessageModel(MessageType.Error, 'You do not have sufficient WXAION balance to withdraw.'), MessageContentType.Text);
    ...
    web3Instance.eth.getBalance(userAccount, (err, result) => {
      ...
      // wx.withdraw(convertedAmount, {from: userAccount}, (err, result) => {
      //   this.transactionInProgress = false;
      //   if (!result || result === undefined || result === null) {
