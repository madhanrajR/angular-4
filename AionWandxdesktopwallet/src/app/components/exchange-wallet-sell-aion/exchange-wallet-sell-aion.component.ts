...
export class ExchangeWalletSellAionComponent implements OnInit {
  ...
  checkAllowance() {
    let userAccount = sessionStorage.getItem("walletAddress");
      ...
      let instanceOrderTraderContract = new this._web3.eth.Contract(Constants.TokenAbiWAN,this.selectedPlatformToken.address, {
        gasLimit: 3000000,
      })  
   ...
   //   console.log('Invalid token address');
    ...
    this._web3.eth.getBlockNumber((err, data) => {
      if (data) {
        ...
        var privatekey=this.web3Service.priv;
        var result=this._web3.eth.accounts.sign([Constants.OrderBookContractAddressWAN,buyTokenAddress, sellTokenAddress, sellVolume, sellPrice,creationBlock + Constants.BlockExpirationWindow, userAccount, 1, sanitizedOrderId,sellerFeeTokenAddress],privatekey)
          console.log(result['messageHash']) 
          if (result) {
            console.log("orderHash",result)
            let publickey='0x'+this.web3Service.priv.substr(66,130)
